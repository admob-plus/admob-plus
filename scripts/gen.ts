...
async function updateConfigXML() {
  const [androidFiles, iosFiles] = await Promise.all([
    glob(['**/*.java'], {
      cwd: path.join(__dirname, '../packages/cordova/src/android'),
    }),
    glob(['*.swift'], {
      cwd: path.join(__dirname, '../packages/cordova/src/ios'),
    }),
  ])
  ...
  const iosContent = iosFiles
    ...
    .join('\n')
  await replaceInFile({
    files: [path.join(__dirname, '../packages/cordova/plugin.xml')],
...
async function main() {
  const l = [
    {
      filepath: 'packages/cordova/src/android/AdSizeType.java',
      f: buildAdSizeTypeJava,
    },
    {
      filepath: 'packages/cordova/src/android/Actions.java',
      f: buildActionsJava,
    },
    {
      filepath: 'packages/cordova/src/android/Events.java',
      f: buildEventsJava,
    },
    {
      filepath: 'packages/cordova/src/ios/AMSConstants.swift',
      f: buildConstantsSwift,
    },
    { filepath: 'packages/core/constants.ts', f: buildConstantsTs },
  ]
  await Promise.all(
    l.map(({ filepath, f }) =>
